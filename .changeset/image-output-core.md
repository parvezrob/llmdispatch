...
Breaking for custom `Provider` implementations: `responseFormat.type` may now be `'image'`; reject it with `ProviderError('invalid_request')` unless you support it. The `anthropic` and `openaiCompatible` adapters do exactly that in this release; the `gemini` adapter maps the image wire instead (see its own entry). Breaking for custom `UsageStore` implementations under the conformance suite: the settled attempt record now carries `usage.imageOutputTokens`, and a store that persists only the two base counters fails the round-trip case.
