...
The core now caps what a response may hand back: more than 32 images, or one image's `data` over 30 000 000 base64 characters, classifies `malformed_response` on the count and the length alone, before the base64 grammar and the image header reader run. Both are payload bounds on the response rather than restatements of the `image.count` knob, which is why the count ceiling sits well above it: one provider candidate may carry several images for a single requested one.
