...
export function createMemoryUsageStore(options: {
  ...
  function reserve(
    ...
    const days = ensureDaysOf(pending, checked)
    ...
    const byDay = ensureDaysOf(counters, checked)
    ...
    return {
      ok: true,
      reservation: { reservationId, key: checked, day },
  ...
  function settle(
    ...
    // Every part of the envelope is read once and it is that reading which is checked, stored
    // and answered with. Checked before the settled test on purpose: a value no store could
    // hold is refused whether or not this reservation has been settled already.
    ...
    if (settlements.has(reservationId)) return
    settlements.set(reservationId, {
      reservation: { reservationId, key, day },
      outcome,
      attempts: records,
    })
  }
  ...
  function snapshot(key: QuotaKey): { used: number; resetsAt: string } {
    ...
    const day = utcDay(new Date(at))
    let lapsed = 0
    for (const id of daysOf(pending, checked)?.get(day) ?? []) {
      const row = reservations.get(id)
      if (row?.state === 'pending' && row.expiresAt <= at) lapsed += 1
    }
    const counter = daysOf(counters, checked)?.get(day)
